...
//        jLabel6.setText(Integer.toString(hour)+"/"+Integer.toString(minute)+"/"+Integer.toString(second));
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        ...
        try{
        String sql="insert into emp (sr_no,name,mobile,date_of_birth,gender,address,username,password,image) values (?,?,?,?,?,?,?,?,?)";
        ps=conn.prepareStatement(sql);
    ...
    try {
        int row=jTable1.getSelectedRow();
        String table_click=(jTable1.getModel().getValueAt(row,0).toString());
        String imagee="select image from emp where sr_no='"+table_click+"'  ";
        ps=conn.prepareStatement(imagee);
        rs=ps.executeQuery();
        if(rs.next()){
        ...
        Logger.getLogger(JTable_data_from_database.class.getName()).log(Level.SEVERE, null, ex);
        ex.printStackTrace();
    }
