...
ResultSet rs=null;
    ...
    private void initComponents() {
        ...
        jComboBox1.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "pressure", "temperature", "volumeflow", "rotationalspeed" }));
...
//        jLabel5.setText(Integer.toString(date)+"/"+Integer.toString(month+1)+"/"+Integer.toString(year));
    ...
    private void jButton5ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton5ActionPerformed
        String val1=jTextField10.getText();
        String val2=jTextField11.getText();
        String tmp=jComboBox1.getSelectedItem().toString();
        if(tmp=="pressure"){
            try{
                String sql="SELECT * FROM user WHERE pressure BETWEEN '"+val1+"' AND '"+val2+"'";
            ps=conn.prepareStatement(sql);
            rs=ps.executeQuery();
            jTable1.setModel(DbUtils.resultSetToTableModel(rs));
        }catch(Exception e){
            JOptionPane.showMessageDialog(this, e);
                ...
                try{
                String sql="SELECT * FROM user WHERE temperature BETWEEN '"+val1+"' AND '"+val2+"'";
            ps=conn.prepareStatement(sql);
            rs=ps.executeQuery();
            jTable1.setModel(DbUtils.resultSetToTableModel(rs));
        }catch(Exception e){
